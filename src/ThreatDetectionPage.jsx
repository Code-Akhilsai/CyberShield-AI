...
const ThreatDetectionPage = () => {
  ...
  // 🔥 State for AI results and upload progress
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      onSnapshot(docRef, (snap) => {
        ...
        if (data?.result) {
          console.log("🧠 Model result received:", data.result);
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-slate-300">
                <li className="flex items-start space-x-2">
                  ...
                  <span>Generates synthetic attack samples for training</span>
                </li>
                <li className="flex items-start space-x-2">
                  <ChevronRight className="w-4 h-4 text-purple-400 mt-0.5 shrink-0" />
                  <span>Random noise vector input transformation</span>
                </li>
                <li className="flex items-start space-x-2">
                  <ChevronRight className="w-4 h-4 text-purple-400 mt-0.5 shrink-0" />
                  <span>Adversarial training for robust detection</span>
                </li>
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-2 text-sm text-slate-300">
                ...
                <li className="flex items-start space-x-2">
                  <ChevronRight className="w-4 h-4 text-cyan-400 mt-0.5 shrink-0" />
                  <span>
                    15-class classification (1 Normal + 14 Attack types)
                  </span>
                </li>
                <li className="flex items-start space-x-2">
                  <ChevronRight className="w-4 h-4 text-cyan-400 mt-0.5 shrink-0" />
                  <span>95% accuracy on Edge-IIoT dataset</span>
                </li>
              </ul>
            </div>
          </div>
